...
def test_is_pre_cropped_image_name_detects_scryfall_prefix():
    assert image.is_pre_cropped_image_name("scryfall_abc_123_front.png") is True
    assert image.is_pre_cropped_image_name("__scryfall_abc_123_back.png") is True
...
def test_effective_dpi_from_dimensions_uses_card_frame_type():
    ...
    scryfall_front_dpi = image.effective_dpi_from_dimensions(
        744, 1038, "scryfall_abc_123_front.png"
    )
    scryfall_back_dpi = image.effective_dpi_from_dimensions(
        744, 1038, "__scryfall_abc_123_back.png"
    )

    assert round(regular_dpi) == 300
    assert round(scryfall_front_dpi) == 300
    assert scryfall_back_dpi == scryfall_front_dpi
...
def test_cropper_skips_crop_for_hidden_scryfall_images(monkeypatch, tmp_path):
    image_dir = tmp_path / "images"
    crop_dir = image_dir / "crop"
    image_dir.mkdir()
    crop_dir.mkdir()
    (image_dir / "__scryfall_abc_123_back.png").write_bytes(b"stub")

    writes = []
    crop_calls = []

    monkeypatch.setattr(image, "list_image_files", lambda folder: ["__scryfall_abc_123_back.png"])
    monkeypatch.setattr(image, "read_image", lambda path: "raw-image")
    monkeypatch.setattr(
        image,
        "write_image",
    ...
    assert crop_calls == []
    assert writes == [(str(crop_dir / "__scryfall_abc_123_back.png"), "raw-image")]
    assert any("Skipping crop for pre-cropped image" in message for message in messages)
